...
export const MergeNotes = ({ templates, notes, onMerge }: MergeNotesProps) => {
  ...
  return (
    <form className="space-y-4 rounded-lg border border-surface-200/50 bg-[color:var(--glass-base)] p-4 shadow-lg shadow-primary-100/40 backdrop-blur dark:border-surface-700 dark:bg-surface-950" onSubmit={handleSubmit}>
      ...
      <div className="space-y-2">
        ...
        <div className="space-y-2">
          {filteredNotes.map((note) => (
            <label
              key={note.id}
              className="flex cursor-pointer items-center justify-between rounded-md border border-surface-200/40 bg-[color:var(--glass-base)] px-3 py-2 text-sm shadow-sm shadow-primary-100/40 transition hover:-translate-y-0.5 dark:border-surface-700 dark:bg-surface-900"
            >
              <div>
      ...
      <button
        type="submit"
        className="rounded-full bg-gradient-to-r from-primary-400 to-accent-200 px-4 py-2 text-sm font-semibold text-[color:var(--text-inverse)] shadow-md transition hover:brightness-105 disabled:cursor-not-allowed disabled:opacity-60"
