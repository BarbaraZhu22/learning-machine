...
const buildDialog = (scenario: DialogScenario) => {
  ...
  const prompts = [
    ...
    `${characterA}: Thanks! ${
      notes ? notes : "I will write this down in the note later."
    }`,
  ];
  return prompts.join("\n");
};
...
export const DialogSimulator = () => {
  ...
  return (
    <div className="space-y-6">
      <form
        className="space-y-4 rounded-lg border border-surface-200/50 bg-[color:var(--glass-base)] p-4 shadow-lg shadow-primary-100/40 backdrop-blur dark:border-surface-700 dark:bg-surface-900"
      ...
      {dialog ? (
        <div className="space-y-4 rounded-lg border border-surface-200/50 bg-[color:var(--glass-base)] p-4 shadow-lg shadow-primary-100/40 backdrop-blur dark:border-surface-700 dark:bg-surface-950">
          <div className="flex items-center justify-between">
            <h4 className="text-base font-semibold text-primary-700 dark:text-primary-200">
              Transcript
            </h4>
            <button
        ...
        <div className="space-y-3 rounded-lg border border-surface-200/50 bg-[color:var(--glass-base)] p-4 shadow-lg shadow-primary-100/40 backdrop-blur dark:border-surface-700 dark:bg-surface-900">
          ...
          <ul className="space-y-2 text-sm">
            {history.map((item) => (
              <li
                key={item.id}
                className="rounded-md border border-surface-200/40 bg-[color:var(--glass-base)] p-3 shadow-sm shadow-primary-100/40 dark:border-surface-700 dark:bg-surface-950"
              >
                <div className="font-semibold">{item.situation}</div>
                <div className="text-xs text-muted-foreground">
                  {item.characterA} & {item.characterB} ·{" "}
